...
public class UnitySO_Generator : EditorWindow
{
    ...
    public class SOInstance
    {
        ...
        public SOInstance(ScriptableObject pSO_Container)
        {
            this.pSO_Container = pSO_Container;
        }
    }
...
#pragma warning restore CS4014 // 이 호출을 대기하지 않으므로 호출이 완료되기 전에 현재 메서드가 계속 실행됩니다.
    ...
    static public void DoUpdate_FromLocalFile()
    {
        ...
        var listTypeData = _pTypeDataList.listTypeData.Where(p => p.bEnable);
        foreach (TypeData pTypeData in listTypeData)
        {
            ...
            _mapSOInstance.Add(pType_SO, new SOInstance(pContainerInstance));
    ...
    private static void InitField_OtherSO()
    {
    ...
    private static void Process_RealField(TypeData pTypeData, Dictionary<string, System.Reflection.FieldInfo> mapFieldInfo_SO, JObject pInstanceData, ScriptableObject pSO, IEnumerable<FieldTypeData> listRealField)
    {
        foreach (var pMember in listRealField)
        {
            ...
            {
                Debug.LogError($"Parsing  Fail - {pTypeData.strFileName}.cs - {pMember.strFieldType} {pMember.strFieldName} : {strValue}\n" +
                    $"Exception : {e}");
            }
        }
    }
            ...
            if (pType_Field.IsEnum)
            {
                ...
                catch (Exception e)
                {
                    if(pTypeData.eType != ESheetType.Global)
                        Debug.LogError($"{pTypeData.strFileName} Enum Parse Error - ({pType_Field.MemberType})\"{strDependencyValue}\"" +
                            $"\n" + e);
                }
            }
            else
